...
public class EnemyVisibleScript : MonoBehaviour
{
	...
	public void SetPlayerCreate() {
		...
		if (target == null)
		{
			target = FindObjectOfType<PlayerMoveScript>().transform;
		}
	}
	...
	public Transform target; //текущий targer
	...
	bool GetRaycast(Vector3 dir)
	{
		...
		{
			if (hit.transform == target)//проверяет посылающий луч с позицией target на карте
			{
				result = true;
	...
	void FixedUpdate()
	{
        switch (playerCreate) // начинает отрисовывать лучи, если есть префаб игрока на сцене
        {
			case PlayerCreate.Create:
					if (Vector3.Distance(transform.position, target.position) < distance + 10)//дистанция отрисовки лучей(чтобы не нагружать сцену вычислениями)
					{
						if (RayToScan())
						{
